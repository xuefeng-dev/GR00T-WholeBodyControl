...
if [[ "$SYSTEM_TYPE" == "jetson" ]]; then
    ...
    if [ -n "$TENSORRT_MOUNT" ]; then
        # Do not bind-mount /usr/lib/aarch64-linux-gnu/nvidia onto itself.
        # NVIDIA Container Runtime owns that path and needs to create driver symlinks there.
        ADDITIONAL_MOUNTS=""
        ...
        # nvidia-container-runtime does not always inject this Jetson DLA compiler library.
        if [ -f "/usr/lib/aarch64-linux-gnu/nvidia/libnvdla_compiler.so" ]; then
            ADDITIONAL_MOUNTS="$ADDITIONAL_MOUNTS -v /usr/lib/aarch64-linux-gnu/nvidia/libnvdla_compiler.so:/usr/lib/aarch64-linux-gnu/nvidia/libnvdla_compiler.so:ro"
        fi
